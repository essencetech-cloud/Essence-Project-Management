...
export async function PATCH(_req: Request, { params }: Params) {
  ...
  const task = await db.task.findUnique({ where: { id: taskId }, select: { project_id: true, title: true, archived_at: true } })
  ...
  const updated = await db.task.update({
    where: { id: taskId },
    data: { archived_at: new Date(), archived_by: session.userId },
  })
